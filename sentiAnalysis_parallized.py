...
''' This function will use most of the functions defined in sentiAnalysis.py '''
...
def load_apply_save(dataDir, outDir='outputFiles',
					chunksize=None, nworker=0, maxchunk=30000):
	"""
...
class Workers:
	...
	def process(self, nworkers):
		...
		for data in self.data_iterator:
			...
			predicted_data_subsets = pool.map(process_by_title, self.data_titles)
			pool.close()
			pool.join()
	...
	def apply_models_DF(self, cleaned_data, model_name, model_dict, cleaned_text_cols):
		...
		for col in cleaned_text_cols:
			texts = cleaned_data[col]
			for task, model in model_dict.items():
				scores = model.predict_proba(texts)[:,1]
				cleaned_data['%s_%s_%s_score' % (col, task, model_name)] = scores
